...
class DiffusionInst(nn.Module):
    ...
    def forward(self, batched_inputs, do_postprocess=True):
        ...
        batched_inputs = [{'file_name': 'xyz', 'height': 12, 'width': 15, 'image_id': 2558, 'image': Tensor, 'instances': Instances}, {}...]
        Instances: 
            num_instances=3
            image_height=512
            image_width=767
            fields=[gt_boxes (N, 4): Boxes(tensor([[267.9467,  46.8234, 308.1902,  73.2867], [ 47.1465, 239.0572,  67.4840, 365.8462], [321.9123, 196.2027, 437.8012, 457.4426]])),
                    gt_classes (N,) : tensor([29,  0,  0]),
                    gt_masks (N, H, W): BitMasks(num_instances=3)]
        ...
        """
        images: (BxCxHxW) Only represents normalized image data in ImageList format (for a batch)
        images_whwh: (Bx4) List of [w, h, w, h] for each image in a batch
        """
        if isinstance(images, (list, torch.Tensor)):
            images = nested_tensor_from_tensor_list(images)

        # Feature Extraction.
        src = self.backbone(images.tensor) # build_resnet_fpn_backbone = https://github.com/facebookresearch/detectron2/blob/main/detectron2/modeling/backbone/fpn.py
        """
        src is a feature pyramid (python dictionary) and keeps p2, p3, p4, p5, p6 as its keys
            src['p2'].shape = [1, 256, 128, 192]
            src['p3'].shape = [1, 256, 128, 96]
            src['p4'].shape = [1, 256, 128, 48]
            src['p5'].shape = [1, 256, 128, 24]
            src['p6'].shape = [1, 256, 128, 12]
        """
        features = list()
        for f in self.in_features: # self.features = [p2, p3, p4, p5]
            feature = src[f] # gets the values from the feature pyramid
        ...
        if self.training:
            gt_instances = [x["instances"].to(self.device) for x in batched_inputs] # len of gt_instances is eq to batch size, 
            # x["instances"] has n_object instances. Instance(gt_boxes, gt_classes, gt_masks)
            # gt_instance: List[Instance]
    ...
    def prepare_targets(self, targets):
        ...
        noises = []
        ts = []
        for targets_per_image in targets: # for instances in list[instances...]
            target = {}
            
            h, w = targets_per_image.image_size
            image_size_xyxy = torch.as_tensor([w, h, w, h], dtype=torch.float, device=self.device)
            gt_classes = targets_per_image.gt_classes # list of numbers(classes), len:num_objects_in_an_image
            gt_boxes = targets_per_image.gt_boxes.tensor / image_size_xyxy # list of bboxes, len:num_objects_in_an_image
    ...
    def preprocess_image(self, batched_inputs):
        ...
        """
        images = [self.normalizer(x["image"].to(self.device)) for x in batched_inputs] # normalizer subtracts mean from each channel and divides to std
        images = ImageList.from_tensors(images, self.size_divisibility) # list of tensors to ImageList class (which is basically list of tensors)

        images_whwh = list()
        for bi in batched_inputs:
            h, w = bi["image"].shape[-2:] # bi["image"].shape was kept in c, h, w format, this line takes the h and w out of every image
            images_whwh.append(torch.tensor([w, h, w, h], dtype=torch.float32, device=self.device))
        images_whwh = torch.stack(images_whwh) # not sure what's happening here
