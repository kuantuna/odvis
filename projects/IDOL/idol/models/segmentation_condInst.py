...
class CondInst_segm(nn.Module):
    ...
    def forward(self, samples, det_targets, ref_targets, criterion, train=False):
        ...
        features, pos = self.detr.backbone(samples) #samples.tensors[0] -> key frame , samples.tensors[1] -> reference frame
        ...
        for l, feat in enumerate(features[1:]): # eliminates the res2 features
            # src: [N, _C, Hi, Wi],
            # mask: [N, Hi, Wi],
            # pos: [N, C, H_p, W_p]
            src, mask = feat.decompose() # used to decompose the images and the masks (key and reference)
            src_proj_l = self.detr.input_proj[l](src)    # src_proj_l: [N, C, Hi, Wi] -- input projecttion -- ne kadar backbone outputu varsa o kadar conv+groupnorm var hepsi için ayrı
            srcs.append(src_proj_l)
            masks.append(mask)
            poses.append(pos[l+1]) # thats why res2 is eliminated i guess
        ...
        hs, memory, init_reference, inter_references, _un1, _un2, _un3 = self.detr.transformer(srcs_key, masks_key, poses_key, query_embeds) # Transformer for the key
        hs_ref, memory_ref, init_reference_ref, inter_references_ref, _un1, _un2, _un3 = self.detr.transformer(srcs_reference, masks_reference, poses_reference, query_embeds) # Transformer for the reference
